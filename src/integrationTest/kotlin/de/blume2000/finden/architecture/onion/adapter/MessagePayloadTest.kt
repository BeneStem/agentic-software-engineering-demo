...
class MessagePayloadTest {
  ...
  companion object {
    const val MESSAGE_PAYLOAD = "MessagePayload"
  }

  @ArchTest
  fun messagePayloadClasses(classes: JavaClasses) =
    classes()
      .that().haveSimpleNameEndingWith(MESSAGE_PAYLOAD)
  ...
  @ArchTest
  fun messagePayloadFields(classes: JavaClasses) =
    fields()
      .that().areDeclaredInClassesThat().haveSimpleNameEndingWith(MESSAGE_PAYLOAD)
